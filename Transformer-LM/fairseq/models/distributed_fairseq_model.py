...
def DistributedFairseqModel(args, model, process_group, device):
    ...
    if args.tpu:
        ...
        train_loader_len = 1576
        powersgd_rank = None
        lgreco_rank = 32
        start_compression = 4000
        adjust_freq = 4000

        if powersgd_rank and not lgreco_rank:
            print("PowerSGD gradient compression", powersgd_rank)
            state = powerSGD.PowerSGDState(torch.distributed.group.WORLD,
                                           matrix_approximation_rank=powersgd_rank, 
                                           start_powerSGD_iter=start_compression)
            wrapped_model.register_comm_hook(state, powerSGD.powerSGD_hook)

        if lgreco_rank and not powersgd_rank:
            print("L-GreCo + PowerSGD gradient compression", lgreco_rank)
            ...
            wrapped_model.register_comm_hook(state, lgreco.powerSGD_hook)
